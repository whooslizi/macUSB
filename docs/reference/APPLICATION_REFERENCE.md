...
- `macUSB/Features/Downloader/*` — downloader module split into `UI/*` (window shell + list/process/summary views), `Logic/*` (discovery + production Catalina/Big Sur/Monterey/Ventura/Sonoma/Sequoia/Tahoe download/verify/assembly/cleanup), and `MacOSDownloaderCoordinator.swift` (window lifecycle/orchestration entry).
